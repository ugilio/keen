...
public abstract class BaseEPSLTab extends AbstractLaunchConfigurationTab {

	protected BaseEPSLComboBlock fEPSLBlock;
    ...
    protected abstract BaseEPSLComboBlock createEPSLComboBlock(boolean defaultFirst);
	...
	public void createControl(Composite parent) {
		...
		fEPSLBlock = createEPSLComboBlock(true);
	...
	private void updatePlannerFromConfig(ILaunchConfiguration config) {
		try {
			...
			IEPSLInstall epsl = getEpslRegistry().getDefault();
			if (epsl != null)
	...
	private IEPSLInstall getEpslForProject() {
		...
		try {
			...
			if (path != null)
				return getEpslRegistry().findByPath(path);
		} catch (CoreException e) {
		}
		return null;
	...
	private EPSLDescriptor getDefaultPlannerDescriptor() {
		return new EPSLDescriptor() {
			...
			public String getDescription() {
				...
				if (epsl != null)
					return String.format(confData.get(ConfigurationData.PROJECT_PLANNER), epsl.getName());

				String name = "undefined";
				epsl = getEpslRegistry().getDefault();
				if (epsl != null)
					name = epsl.getName();
				return String.format(confData.get(ConfigurationData.WORKSPACE_DEFAULT_PLANNER), name);
			}
		};
	}
