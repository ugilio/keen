...
public abstract class BaseInstalledEPSLsBlock implements ISelectionProvider {
    ...
    protected abstract BaseEpslRegistry getEpslRegistry();
    
    protected abstract BaseAddEpslInstallWizard createAddWizard(IEPSLInstall[] currentInstalls);
    
	protected abstract EpslInstallWizard createEditWizard(EpslInstallImpl vm, IEPSLInstall[] allVMs);
	...
	public void createControl(Composite ancestor) {
		...
		SWTFactory.createLabel(parent, getTitle(), 2);
	...
	protected void copyInstall() {
		...
		while (it.hasNext()) {
			...
			EpslInstallWizard wizard = createEditWizard(epsl,
	...
	private void addInstall() {
		BaseAddEpslInstallWizard wizard = createAddWizard(fPlanners.toArray(new IEPSLInstall[fPlanners.size()]));
		WizardDialog dialog = new WizardDialog(getShell(), wizard);
	...
	private String createUniqueId() {
		...
		} while (getEpslRegistry().findById(id) != null || id.equals(fgLastUsedID));
		fgLastUsedID = id;
		return id;
	}
	...
	private boolean validatePlannerLocation(File directory) {
		File f = new File(directory.getPath(), getEpslRegistry().getJarName());
	...
	protected void fillWithWorkspacePlanners() {
		setPlanners(getEpslRegistry().getInstalledPlanners());
	}
}
