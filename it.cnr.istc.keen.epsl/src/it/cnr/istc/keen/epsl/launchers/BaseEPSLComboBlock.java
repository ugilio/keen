...
public abstract class BaseEPSLComboBlock {
	...
	private BaseEpslRegistry fEpslRegistry = null;
	private ConfigurationData confData = null;
	...
	private static IStatus OK_STATUS = new Status(IStatus.OK, Activator.PLUGIN_ID, ""); //$NON-NLS-1$

	public BaseEPSLComboBlock(boolean defaultFirst) {
		fDefaultFirst = defaultFirst;
		confData = getConfigurationData();
		fEpslRegistry = getEpslRegistry();
	}
	...
	private void firePropertyChange() {
		PropertyChangeEvent event = new PropertyChangeEvent(this, getPropertyName(), null, getPath());
	...
	public void createControl(Composite ancestor) {
		...
		if (fTitle == null) {
			fTitle = confData.get(ConfigurationData.PLANNER_TO_RUN);
		}
	...
	private void createSpecificPlannerControls(Composite comp) {
		String text = confData.get(ConfigurationData.SPECIFIC_PLANNER);
			...
			public void widgetSelected(SelectionEvent e) {
				if (fSpecificButton.getSelection()) {
					...
					if (fPlanners.isEmpty()) {
						setError(confData.get(ConfigurationData.NO_PLANNERS));
					} else {
						setStatus(OK_STATUS);
					}
					firePropertyChange();
		...
		fManageButton = SWTFactory.createPushButton(comp, confData.get(ConfigurationData.INSTALLED_PLANNERS), null);
		fManageButton.addListener(SWT.Selection, new Listener() {
			@Override
			public void handleEvent(Event event) {
				showPrefPage(getPreferencesPageID());
			}
		});
	...
	protected void fillWithWorkspacePlanners() {
		...
		IEPSLInstall[] installs = fEpslRegistry.getInstalledPlanners();
	...
	public void setPath(IPath containerPath) {
		fErrorPath = null;
		setStatus(OK_STATUS);
		IEPSLInstall install = containerPath != null ? fEpslRegistry.findByPath(containerPath.toOSString()) : null;
		if (install != null && fEpslRegistry.getDefault().equals(install)) {
			setUseDefaultPlanner();
		} else {
			if (install == null) {
				selectPlanner(install);
				fErrorPath = containerPath;
				setError(confData.get(ConfigurationData.PLANNER_NOTFOUND));
			} else {
				selectPlanner(install);
				File location = install.getInstallLocation();
				if (location == null) {
					setError(confData.get(ConfigurationData.HOME_NOTFOUND));
				} else if (!location.exists()) {
					setError(confData.get(ConfigurationData.HOME_NOTEXIST));
				}
			}
		}
	}
